...
import { Plus, Search, Package, AlertTriangle, Edit, Loader2, Trash2 } from 'lucide-react';
...
const PartForm = ({ initialData, onSubmit, onCancel, isSubmitting }) => {
  ...
  useEffect(() => {
     if (initialData) {
      setFormData({
        ...
        partNumber: initialData.partnumber || '', // db column name
        category: initialData.category || categories[0],
        supplier: initialData.supplier || '',
        quantity: initialData.quantity === null || initialData.quantity === undefined ? 0 : Number(initialData.quantity),
        minQuantity: initialData.minquantity === null || initialData.minquantity === undefined ? 5 : Number(initialData.minquantity), // db column name
        unitCost: initialData.unitcost === null || initialData.unitcost === undefined ? 0 : Number(initialData.unitcost), // db column name
  ...
  const handleSubmitForm = (e) => {
    ...
    const dataToSubmit = {
      name: formData.name,
      partnumber: formData.partNumber, // db column name
      category: formData.category,
      supplier: formData.supplier || null,
      quantity: formData.quantity === null || formData.quantity === undefined ? 0 : Number(formData.quantity),
      minquantity: formData.minQuantity === null || formData.minQuantity === undefined ? 0 : Number(formData.minQuantity), // db column name
      unitcost: formData.unitCost === null || formData.unitCost === undefined ? 0 : Number(formData.unitCost), // db column name
...
const PartCard = ({ part, onEdit, onDelete }) => {
  ...
  const minQuantity = part.minquantity === null || part.minquantity === undefined ? 0 : Number(part.minquantity); // db column name
  const unitCost = part.unitcost === null || part.unitcost === undefined ? 0 : Number(part.unitcost); // db column name
      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button variant="ghost" size="icon" onClick={() => onEdit(part)} className="h-8 w-8 text-muted-foreground hover:text-primary"><Edit className="h-4 w-4" /></Button>
          </div>
        </div>
      </CardHeader>
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={() => onDelete(part.id)}>Excluir</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
...
const PartsManager = ({ parts: initialParts, setParts: setGlobalParts }) => {
  ...
  const filteredParts = parts.filter(part => {
    ...
    const partNumber = part.partnumber || ""; // db column name
    ...
    try {
      if (editingPart) {
        const { data, error } = await supabase.from('parts')
          .update({ ...partData, updatedat: new Date().toISOString() })
          .eq('id', editingPart.id)
          .select()
          .single();
        if (error) throw error;
        setGlobalParts(prev => prev.map(p => p.id === editingPart.id ? data : p));
        toast({ title: "Sucesso!", description: "Peça atualizada com êxito." });
      } else {
        const { data, error } = await supabase.from('parts')
          .insert([{ ...partData, id: crypto.randomUUID(), createdat: new Date().toISOString(), updatedat: new Date().toISOString() }])
          .select()
          .single();
        if (error) throw error;
        setGlobalParts(prev => [data, ...prev].sort((a,b) => new Date(b.createdat) - new Date(a.createdat)));
        toast({ title: "Sucesso!", description: "Nova peça cadastrada." });
      }
      setIsDialogOpen(false);
      setEditingPart(null);
    } catch (error) {
      console.error('Error submitting part:', error);
      toast({ title: "Erro ao Salvar", description: `Falha ao salvar peça: ${error.message}`, variant: "destructive" });
    } finally {
  ...
  const handleDeletePart = async (partId) => {
    try {
      ...
      if (error) throw error;
      setGlobalParts(prev => prev.filter(p => p.id !== partId));
      toast({ title: "Sucesso!", description: "Peça excluída." });
    } catch (error) {
      console.error('Error deleting part:', error);
      toast({ title: "Erro ao Excluir", description: `Falha ao excluir peça: ${error.message}`, variant: "destructive" });
    }
  };

  const openAddDialog = () => { setEditingPart(null); setIsDialogOpen(true); };
  const openEditDialog = (part) => { setEditingPart(part); setIsDialogOpen(true); };
  
  const lowStockPartsCount = parts.filter(part => {
    const quantity = part.quantity === null || part.quantity === undefined ? 0 : Number(part.quantity);
    const minQuantity = part.minquantity === null || part.minquantity === undefined ? 0 : Number(part.minquantity); // db column name
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-border">
        <div><h1 className="text-3xl md:text-4xl font-bold text-foreground">Gestão de Peças</h1><p className="text-muted-foreground text-base md:text-lg">Controle seu estoque de peças e componentes.</p></div>
        <Dialog open={isDialogOpen} onOpenChange={(isOpen) => { if (!isSubmitting) setIsDialogOpen(isOpen); }}>
          <DialogTrigger asChild><Button onClick={openAddDialog} className="bg-primary hover:bg-primary/90 text-primary-foreground"><Plus className="h-4 w-4 mr-2" /> Nova Peça</Button></DialogTrigger>
          <DialogContent className="max-w-2xl"><DialogHeader><DialogTitle className="text-foreground">{editingPart ? 'Editar Peça' : 'Cadastrar Nova Peça'}</DialogTitle></DialogHeader><PartForm initialData={editingPart} onSubmit={handleFormSubmit} onCancel={() => setIsDialogOpen(false)} isSubmitting={isSubmitting} /></DialogContent>
        </Dialog>
      </motion.div>
      ...
      {lowStockPartsCount > 0 && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}><Card className="glass-effect border-yellow-500/50 dark:border-yellow-400/30"><CardHeader><CardTitle className="flex items-center gap-2 text-yellow-600 dark:text-yellow-400"><AlertTriangle className="h-5 w-5" /> Alerta de Estoque Baixo</CardTitle></CardHeader><CardContent><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-60 overflow-y-auto scrollbar-hide">{parts.filter(p => { const quant = p.quantity === null || p.quantity === undefined ? 0 : Number(p.quantity); const minQuant = p.minquantity === null || p.minquantity === undefined ? 0 : Number(p.minquantity); return quant <= minQuant; }).slice(0, 6).map((part) => (<div key={part.id} className="flex items-center justify-between p-3 rounded-md bg-yellow-500/10 dark:bg-yellow-400/10 border border-yellow-500/20 dark:border-yellow-400/20"><div><p className="font-medium text-foreground">{part.name}</p><p className="text-sm text-muted-foreground">{part.partnumber}</p></div><Badge variant="outline" className="border-yellow-500 text-yellow-600 dark:border-yellow-400 dark:text-yellow-300">{part.quantity === null || part.quantity === undefined ? 0 : part.quantity} un.</Badge></div>))}</div></CardContent></Card></motion.div>)}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">{filteredParts.map((part, index) => (<motion.div key={part.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.03, duration: 0.3 }}><PartCard part={part} onEdit={openEditDialog} onDelete={handleDeletePart} /></motion.div>))}</div>
